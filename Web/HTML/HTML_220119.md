...
- 블록요소 : 자기 태그가 있는 화면 처음부터 화면 끝까지 자신의 영역으로 가지고 있음. 새로운 태그를 쓰면 __자동으로 줄을 바꾼다__ 왜냐하면 끝까지 내 영역이니까. 각 태그마다 블록요소와 인라인 요소가 나뉘어져있음. h1, h2 등이 블락요소이다. 심지어 위아래로 blank도 차지하고 있다.
...
:heavy_check_mark: 꺽새를 표현하고 싶을 때 : `(&lt;텍스트b&gt;)` ==> `<small>작은텍스트(%&lt;small&gt;)`
...
- `<div>` : 영역 정의. 공간 위를 차지하지 않는다. div는 p를 포함할 수 있다. 용도는 __HTML 영역별 구분__
  - 후에 나오는 article, nave 등의 태그도 div의 연장선이다
...
- `<style type="text/css"> `:text파일인데 css로 불러왔다
...
- div로 섹션들을 일일이 나누고 id로 가독성있게 레이아웃 생성
...
- 시멘틱 태그  : form, table, article처럼 태그명의 뜻이 직관적으로 이해할 수 있는 이름
...
- 폼에서 쓰이는 컨트롤과 비슷하다
- 마찬가지로 `form` 태그 활용한다
- form에 컨트롤과는 다르게, __ select와 option__태그를 활용한다
- 대신 밑에 submit의 input을 작성은 해줘야한다
- `<select name=>` : name은 for 서버, select하나당 하나의 select 컨트롤이 만들어진다
  - `<optgroup label=>` 하나의 셀렉트에서 있는 소그룹, label은 사용자가 보는 이름
